...
def trend_stats(counts: pd.DataFrame, shares: pd.DataFrame, batch_meta: pd.DataFrame, cfg: dict, label_name: str):
    bm = batch_meta.set_index("batch")
    batches = list(counts.columns)
    full = [b for b in batches if not bm.loc[b, "is_partial_batch"]]
    ...
    for lab in counts.index:
        ...
        r = {label_name: lab, "total_companies": int(c.sum()),
